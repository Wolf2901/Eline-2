...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, Amelia deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By AmeliaAI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, Amelia deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By AmeliaAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
"""
...
# Full Credits goes to @Elina_Robot
...
__help__ = """
<b> Group Guardian: </b>
✪ Elina can protect your group from NSFW senders, Slag word users and also can force members to use English
...
Note: Special credits goes to @Wolf_2904
